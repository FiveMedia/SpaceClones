...
public class SlimeSpriteBig extends WalkingEnemySprite {
    ...
    int m_lightDistance = 750;
    boolean m_destroyStuff = false;
    int m_iter  = 0;
    Rectangle m_bb = new Rectangle();
    float m_xOff, m_yOff;

    public SlimeSpriteBig(TextureAtlas myTextures, TiledMapTileLayer pTiles, TiledMapTileLayer lTiles, float sp, MainGameLayer layer, int v, float scale, int delayTicks) {
    super(myTextures.findRegion("slime_e_b_F1"),pTiles,lTiles, 2f * sp, 12f,v, scale, true, delayTicks);
    float tt = 0.4f / sp;
    m_glowAnimation = new AnimateSpriteFrame(myTextures, new String[] {"slime_e_b_F1", "slime_e_b_F2"}, tt, 3);
    m_walkAnimation = new AnimateSpriteFrame(myTextures, new String[] {"slime_e_b_F1", "slime_e_b_F2"}, tt/3, 1);

    m_standardDeathAnimation = new AnimateSpriteFrame(myTextures, new String[] {"slime_e_b_F1", "slime_e_b_F2",  "slime_e_b_F1", "slime_e_b_F2"}, 0.45f, 1);
    m_hitAnimation = new AnimateSpriteFrame(myTextures, new String[] {"slime_e_b_F1", "slime_e_b_F2",  "slime_e_b_F1", "slime_e_b_F2"}, 0.25f, 1);
  ...
  public void destroySprites()
  {
    ...
    for (m_iter = 0; m_iter < m_gameLayer.getChildren().size(); m_iter++)
    {
      ...
      if (((s instanceof TreeSprite) || (s instanceof MovingBlock) || (s instanceof SwitchSprite) || (s instanceof SlimeSprite)) && (s.isVisible()))
      {
        if (s != this)
        {
          ...
          if (Intersector.overlaps(ss.getBoundingRectangle(), m_bb))
          {
            if (s instanceof MovingBlock)
            {
              ...
              sss.die();
            } else if (s instanceof SlimeSprite)
            {
              SlimeSprite sss = (SlimeSprite)s;
              sss.die();
            } else
            {
              ss.setVisible(false);
  ...
  {
    float xx = this.getX() + (this.getWidth() * m_scale);
